...
tstamp <- str_replace_all(end_sims, " ", "_") %>% 
  str_replace_all(":", "") |> 
  str_replace("[.]", "_")
dir.create(paste0("appendix/sim_output/", lubridate::date(tstamp)))
write_csv(x = dynamics_total, col_names = TRUE, 
          file = here(paste0("appendix/sim_output/", lubridate::date(tstamp), "/diversity_partitioning_disp_kernel_20sp_", tstamp ,"_summary.csv")))
write_csv(x = spat_dyn_over_time_total, col_names = TRUE, 
          file = here(paste0("appendix/sim_output/", lubridate::date(tstamp), "/diversity_partitioning_disp_kernel_20sp_", tstamp ,"_spat_per_time.csv")))
write_csv(x = temp_dyn_per_patch_total, col_names = TRUE, 
          file = here(paste0("appendix/sim_output/", lubridate::date(tstamp), "/diversity_partitioning_disp_kernel_20sp_", tstamp ,"_temp_per_patch.csv")))
