                                   ...
                                   .packages = c("tidyverse", "data.table", "stats")) %dopar% {
                                     ...
                                     try(expr = {
                                       ...
                                       for(i in 1:(initialization + burn_in + timesteps)){
                                         ...
                                         fitness_biotic <- N_hat - N_germ # delta N after accounting for germination numbers in each patch
                                       ...
                                       dynamics_out |> 
                                         filter(species == 39) |> count(stoch_extinct_env)
                                         ggplot(aes(x = W_bio, y = N, color = as.factor(species))) +
                                         geom_point() +
                                         geom_smooth()
                                       ...
                                       dynamics_out |> 
                                         filter(N > 0) |> 
                                         ggplot(aes(x = time, y = W_bio, color = as.factor(species))) +
                                         geom_point() +
                                         facet_wrap(~patch)
